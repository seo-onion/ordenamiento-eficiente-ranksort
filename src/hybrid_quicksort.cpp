...
#pragma omp threadprivate(local_compare_count)
...
void quicksort_tasks(Complex* A, int low, int high) {
    if (low < high) {
        // Threshold para evitar overhead de tareas en subarreglos pequeños
        if (high - low < 1000) { 
            quicksort_seq(A, low, high);
        } else {
            int p = median_of_three(A, low, high);
            
            #pragma omp task shared(A)
            quicksort_tasks(A, low, p - 1);
            
            #pragma omp task shared(A)
            quicksort_tasks(A, p + 1, high);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    if (provided < MPI_THREAD_FUNNELED) {
        std::cerr << "Advertencia: Nivel de soporte de hilos MPI insuficiente." << '\n';
    }
    ...
    if (rank == 0) {
        ...
        if (!filename.empty()) {
            if (read_binary_file(filename, A, N)) {
                std::cout << "Hybrid Quicksort PSRS leyendo binario: " << filename << " con N = " << N << '\n';
            } else {
            ...
            std::cout << "Hybrid Quicksort PSRS inicializado (aleatorio) con N = " << N << '\n';
        }
    ...
    // --- Fase 1: Ordenamiento Local con OpenMP Tasks ---
    double t_comp_sort_start = MPI_Wtime();
    long long total_local_comparisons = 0;
    
    #pragma omp parallel
    {
        local_compare_count = 0; // Inicializar variable threadprivate
        
        #pragma omp single nowait
        {
            quicksort_tasks(local_A.data(), 0, local_n - 1);
        }
        
        #pragma omp taskwait
        
        #pragma omp atomic
        total_local_comparisons += local_compare_count;
    }
    
    long long local_flops = total_local_comparisons * 6;
    ...
    if (size == 1) {
        ...
        if (rank == 0) {
            std::cout << "\n--- Resultados de la Ejecución (Hybrid Quicksort PSRS - Secuencial/Single Process) ---" << '\n';
    ...
    local_flops += merge_comparisons * 6;
    ...
    if (rank == 0) {
        ...
        std::cout << "\n--- Resultados de la Ejecución (Hybrid Quicksort PSRS) ---" << '\n';
        std::cout << "Ordenamiento correcto: " << (sorted ? "SÍ" : "NO") << '\n';
        ...
        std::cout << "  - Quicksort local (OpenMP Tasks): " << t_local_sort << " s" << '\n';
